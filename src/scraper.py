...
class Scraper:
    ...
    def _get_sleep_time(self):
        ...
        if self.consecutive_errors >= self.max_consecutive_errors:
            ...
            print(
                f"Backing off - new sleep range: {self.min_sleep:.2f}s to {self.max_sleep:.2f}s"
            )
    ...
    def scrape_website(self, first_page_data=None, total_estates=None):
        ...
        if first_page_data is None:
            ...
            for word in words:
                ...
                total_estates = len(
                    first_page_data
                )  # Fallback to first page count if we can't find total
        else:
            estates += first_page_data
        ...
        # Create progress bar
        with tqdm(
            total=total_pages, desc="Scraping pages", unit="page", ncols=100
        ) as pbar:
